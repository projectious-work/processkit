...
- **`src/skills/FORMAT.md`** — the skill package format spec
...
cd /workspace
ls src/                      # primitives, skills, processes, packages, lib, INDEX.md
```
...
1. Decide the **category** (`process`, `language`, `framework`, ...) and
   **layer** (`null` for technical skills, 0–4 for process-primitive
   skills).
2. Create `src/skills/<skill-name>/SKILL.md` with the frontmatter from
   `src/skills/FORMAT.md`. Use the three-level structure:
...
5. If the skill is a process-primitive skill that should appear in a
   package tier, add it to `src/packages/managed.yaml` (or higher).
6. Update `src/skills/INDEX.md` if appropriate.
7. Add the skill to `docs-site/sidebars.js` under the right catalog
   page if it deserves a docs entry.

## Adding a new primitive

1. Add `src/primitives/schemas/<kind-lowercase>.yaml` (a Schema entity
   with `spec.spec_schema` = JSON Schema for the primitive's spec).
2. Update `src/primitives/INDEX.md` and `src/primitives/FORMAT.md` kind
   registry table.
3. Update `src/lib/processkit/__init__.py` `KIND_PREFIXES` and
   `DEFAULT_DIRS`.
4. If the primitive has lifecycle, add a state machine at
   `src/primitives/state-machines/<name>.yaml`.
...
1. Create `src/skills/<skill>/mcp/server.py` using the boilerplate from
   any existing server (e.g. `event-log/mcp/server.py`).
...
`src/lib/processkit/` is internal infrastructure. Public modules:
