...
- **Using notes as a dumping ground for everything instead of a capture layer for non-backlog ideas.** Notes are for ideas not yet ready to be backlog items. Things that are clearly tasks should go directly into BACKLOG.md. Things that are clearly decisions should go into decisions-adr. Notes are the intermediate capture for things that haven't crystallized yet — not a general-purpose inbox for all project work.
