...
def _historical_path(root: Path, path: Path) -> bool:
    """Return whether a path is append-only or immutable history."""
...
def _rewrite_active_references(
    ...
    """Replace exact IDs in mutable files while preserving history."""
    changed: list[str] = []
...
def draft_migration(
    ...
    """Create a pending, locally-authored Migration entity.

    Use this for schema extensions and auditable data repairs that cannot be
    generated by an installer. Each ``affected_files`` entry requires
    ``path`` and ``classification`` strings. Review the returned pending
    Migration before applying it.

    Prerequisite: call find_skill(task_description) or confirm a named
    processkit skill. Call route_task before this write and commit the
    resulting entity in the same turn.
    """
    valid_kinds = {"schema-extension", "data-fix", "runtime", "content"}
    if kind not in valid_kinds:
        return {"error": f"invalid kind {kind!r}; must be one of {sorted(valid_kinds)}"}
    ...
    if not all(
        ...
        return {"error": "affected_files entries require string path and classification"}

    root = paths.find_project_root()
    cfg = config.load_config(root)
    db = index.open_db()
    try:
        existing = index.existing_ids(db, "Migration")
    finally:
        db.close()
    new_id = ids.generate_id(
        ...
        slug_text=summary if cfg.id_slug else None,
        existing=existing,
    )
    now = _now_iso()
    spec: dict[str, Any] = {
        ...
        "source_processkit_version": PK_VERSION,
        "target_api_version": API_VERSION,
        "target_processkit_version": PK_VERSION,
        "apply_mode": "automatic",
    }
    if related_decisions:
        spec["related_decisions"] = related_decisions
    errors = schema.validate_spec("Migration", spec)
    if errors:
    ...
    default_body = (
        ...
        f"{summary.strip()}\n\n"
        "## Plan\n\n"
        "Review and complete this plan before applying the migration.\n"
    )
    ent = entity.new(
        "Migration", new_id, spec, body=body.strip() or default_body
    )
    target = _state_dir(root, "pending") / f"{new_id}.md"
    ...
    event_id = log.log_side_effect(
        ...
        new_id,
        "migration.drafted",
        f"Drafted Migration {new_id!r}: {summary.strip()}",
        root=root,
...
def normalize_migration_filename(id: str, target_id: str) -> dict:
    """Normalize a Migration filename and ID with an audit bridge.

    The target must match ``MIG-YYYYMMDD_HHMM-WordPair``. The corrected
    entity is written before the old file is removed. Mutable references and
    indexes are refreshed, while LogEntries and terminal Migration history
    remain append-only and are bridged by a new normalization event.

    Prerequisite: call find_skill(task_description) or confirm a named
    processkit skill. Call route_task before this write and commit all
    resulting changes in the same turn.
    """
...
def plan_v0_to_v1_migration(
    source_version: str = "v0.28.3",
    target_version: str = "v1.0.0-alpha.1",
) -> dict:
    """Plan the deterministic v0-to-v1 ontology migration without writing.

    The returned plan contains source hashes and must be passed unchanged to
    ``execute_v0_to_v1_migration``. Prerequisite: call
    find_skill(task_description) or confirm a named processkit skill.
    """
    return corpus_migration.plan_v0_to_v1(
...
def execute_v0_to_v1_migration(plan: dict[str, Any]) -> dict:
    """Execute a previously returned, hash-guarded v0-to-v1 plan.

    Prerequisite: call find_skill(task_description) or confirm you are
    already operating within a named processkit skill. 1% rule: call
    route_task first; commit in the same turn — deferred writes are dropped.
    """
    root = paths.find_project_root()
    try:
        report = corpus_migration.execute_v0_to_v1(root, plan)
    except corpus_migration.CorpusMigrationError as exc:
        return {"error": str(exc)}
    event_id = log.log_side_effect(
        "Migration",
        plan.get("plan_id", "v0-v1"),
        "migration.v0-v1-corpus-applied",
        f"Applied v0-to-v1 corpus plan {plan.get('plan_id')!r}",
        root=root,
        actor="processkit-migration-management",
        details=report,
    )
