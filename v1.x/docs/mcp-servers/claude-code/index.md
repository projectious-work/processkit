...
- `.claude/settings.example.json` was the natural home for the
  ...
  `docs-site/content/en/docs/mcp-servers/claude-code/settings.example.json`
  instead and must be copied into `.claude/settings.json` (or the
  user-level config) by hand. If/when sandbox policy permits, the
  canonical location is `.claude/settings.example.json`.
