...
**Status:** pending
**aibox CLI:** v0.25.9 → v0.25.10
**processkit:** v0.26.2

## Summary

aibox has been updated from v0.25.9 to v0.25.10. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.25.9 and v0.25.10 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.25.10** (processkit v0.26.2): Patch release: integrates processkit v0.26.2; adds configurable tmux status labels/layouts and model-provider health segments; reduces PowerKit refresh churn; fixes log-viewer Vim yanking through the tmux/host clipboard bridge; stabilizes runtime MCP diagnostics; verifies release checksum sidecars; and improves image layer/cache reuse across apply/up and host release publishing.
  <https://github.com/projectious-work/aibox/releases/tag/v0.25.10>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox apply` was run for v0.25.10 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.25.10` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.25.10"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.25.10.
