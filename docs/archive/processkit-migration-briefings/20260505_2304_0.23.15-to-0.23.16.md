...
**Status:** pending
**aibox CLI:** v0.23.15 → v0.23.16
**processkit:** v0.25.8

## Summary

aibox has been updated from v0.23.15 to v0.23.16. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.23.15 and v0.23.16 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.23.16** (processkit v0.25.8): Patch release: moves Claude processkit command shims to Claude Code's current Skills layout, cleans legacy managed .claude/commands files, fixes the native Zellij key-hint row render trigger, keeps Vim editor panes hot so Yazi edit handoff reaches Vim instead of bash, and adds a pre-release dependency/harness state report.
  <https://github.com/projectious-work/aibox/releases/tag/v0.23.16>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox apply` was run for v0.23.16 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.23.16` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.23.16"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.23.16.
