...
**Status:** completed
**aibox CLI:** v0.17.6 → v0.17.9
**processkit:** v0.8.0

## Summary

aibox has been updated from v0.17.6 to v0.17.9. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Review the release notes for each version in this upgrade:
- https://github.com/projectious-work/aibox/releases/tag/v0.17.7
- https://github.com/projectious-work/aibox/releases/tag/v0.17.8
- https://github.com/projectious-work/aibox/releases/tag/v0.17.9

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox sync` was run for v0.17.9 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.17.9` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.17.9"`
...
- [ ] Verify `/workspace/context/templates/processkit/v0.8.0/` snapshot directory exists
...
processkit is pinned to `v0.8.0` (source: `https://github.com/projectious-work/processkit.git`).
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.17.9.
