...
**Status:** pending
**aibox CLI:** v0.25.10 → v0.25.11
**processkit:** v0.26.2

## Summary

aibox has been updated from v0.25.10 to v0.25.11. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.25.10 and v0.25.11 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.25.11** (processkit v0.26.2): Patch release: keeps processkit v0.26.2 and fixes source-checkout addon discovery, PowerKit status cache writability, generated runtime writability diagnostics, disabled-harness migration schema metadata, and tmux/Yazi status glyph defaults.
  <https://github.com/projectious-work/aibox/releases/tag/v0.25.11>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox apply` was run for v0.25.11 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.25.11` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.25.11"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.25.11.
