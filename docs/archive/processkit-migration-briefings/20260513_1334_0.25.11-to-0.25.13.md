...
**Status:** pending
**aibox CLI:** v0.25.11 → v0.25.13
**processkit:** v0.26.2

## Summary

aibox has been updated from v0.25.11 to v0.25.13. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.25.11 and v0.25.13 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.25.12** (processkit v0.26.2): Patch release: keeps processkit v0.26.2 and fixes fresh-project tmux/PowerKit runtime projection by using broad writable runtime-home mounts, refreshing managed theme/Yazi/Codex files on apply, scoping preauth/MCP writes to enabled harnesses, recognizing nested processkit skill catalogs in doctor, and reducing host-side doctor probes for container-owned dependencies.
  <https://github.com/projectious-work/aibox/releases/tag/v0.25.12>
- **v0.25.13** (processkit v0.26.2): Patch release: keeps processkit v0.26.2 and fixes stale runtime-home propagation by making aibox-managed .aibox-home files authoritative on apply and clean runtime recreation; broadens generated runtime mounts for Vim and Cargo cache directories; updates generated compose/docs coverage; and adds regression coverage for stale tmux/Yazi managed file refresh.
  <https://github.com/projectious-work/aibox/releases/tag/v0.25.13>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox apply` was run for v0.25.13 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.25.13` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.25.13"`
...
- [ ] Verify `/workspace/context/templates/processkit/v0.26.2/` snapshot directory exists
...
- [ ] Check `/workspace/context/migrations/pending/` for processkit content migration files:
      ...
      - **When reviewing template files like `AGENTS.md`:** do NOT diff the installed
        ...
        `diff context/templates/processkit/v0.26.2/AGENTS.md context/templates/processkit/v0.26.2/AGENTS.md`
...
processkit is tracking `version = "latest"` — installed: `v0.26.2` (upgraded from `v0.26.2` — aibox.toml now targets `vlatest`) (source: `https://github.com/projectious-work/processkit.git`).
...
- [ ] processkit version reference matches `aibox.lock` (`v0.26.2`)
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.25.13.
