...
**aibox CLI:** v0.18.7 → v0.19.0
**processkit:** v0.21.0

## Summary

aibox has been updated from v0.18.7 to v0.19.0. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.18.7 and v0.19.0 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.19.0** (processkit v0.21.0): Minor release: integrates processkit v0.21.0 (major upstream update with enhanced content structure); ships global MCP permission configuration across 8 harnesses (Claude Code, OpenCode, Continue, Cursor, Gemini, Copilot, Aider, Codex) via [mcp.permissions] in aibox.toml with glob pattern matching and deny-precedence semantics; completed backlog grooming with 90-day focus established; all tests passing (597 unit + 41 E2E + 16 integration).
  <https://github.com/projectious-work/aibox/releases/tag/v0.19.0>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox sync` was run for v0.19.0 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.19.0` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.19.0"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.19.0.
