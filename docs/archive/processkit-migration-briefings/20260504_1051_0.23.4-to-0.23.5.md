...
**Status:** completed
**aibox CLI:** v0.23.4 → v0.23.5
**processkit:** v0.25.4

## Summary

aibox has been updated from v0.23.4 to v0.23.5. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.23.4 and v0.23.5 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.23.5** (processkit v0.25.4): Patch release: fixes generated Dockerfile lazygit disablement cleanup so a missing apt package no longer aborts `aibox apply --no-cache`, while still removing lazygit binaries inherited from older base images.
  <https://github.com/projectious-work/aibox/releases/tag/v0.23.5>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox apply` was run for v0.23.5 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.23.5` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.23.5"`
...
with processkit `v0.25.4`; required processkit skills, schemas, and
template snapshots are present. No processkit Migration entities remain
pending or in progress. `context/processes/` is absent in the current
`v0.25.4` snapshot, so that generated checklist item is stale for this
project.
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.23.5.
