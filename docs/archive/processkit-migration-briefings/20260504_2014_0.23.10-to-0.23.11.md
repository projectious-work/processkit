...
**Status:** pending
**aibox CLI:** v0.23.10 → v0.23.11
**processkit:** v0.25.7

## Summary

aibox has been updated from v0.23.10 to v0.23.11. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.23.10 and v0.23.11 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.23.11** (processkit v0.25.7): Patch release: groups generated aibox.toml around aibox, container, processkit, and ai ownership boundaries; adds catalog-style AI harness and model-provider controls; exposes generated path settings; defaults new projects to the product skill set; and repairs managed Zellij status runtime files during sync.
  <https://github.com/projectious-work/aibox/releases/tag/v0.23.11>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox apply` was run for v0.23.11 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.23.11` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.23.11"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.23.11.
