...
**Status:** completed
**aibox CLI:** v0.23.6 → v0.23.7
**processkit:** v0.25.5

## Summary

aibox has been updated from v0.23.6 to v0.23.7. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.23.6 and v0.23.7 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.23.7** (processkit v0.25.5): Patch release: fixes host-generated Codex processkit-gateway MCP paths for devcontainer runtimes, keeping subagent-safe absolute paths while targeting the container workspace mount; doctor now warns about stale host-side Codex MCP script paths.
  <https://github.com/projectious-work/aibox/releases/tag/v0.23.7>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox apply` was run for v0.23.7 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.23.7` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.23.7"`
...
- [ ] Verify `/workspace/context/templates/processkit/v0.25.5/` snapshot directory exists
...
- [ ] Check `/workspace/context/migrations/pending/` for processkit content migration files:
      ...
      - **When reviewing template files like `AGENTS.md`:** do NOT diff the installed
        ...
        `diff context/templates/processkit/v0.25.5/AGENTS.md context/templates/processkit/v0.25.5/AGENTS.md`
...
processkit is tracking `version = "latest"` — installed: `v0.25.5` (upgraded from `v0.25.5` — aibox.toml now targets `vlatest`) (source: `https://github.com/projectious-work/processkit.git`).
...
- [ ] processkit version reference matches `aibox.lock` (`v0.25.5`)
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.23.7.
