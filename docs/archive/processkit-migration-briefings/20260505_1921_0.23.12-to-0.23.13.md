...
**Status:** pending
**aibox CLI:** v0.23.12 → v0.23.13
**processkit:** v0.25.8

## Summary

aibox has been updated from v0.23.12 to v0.23.13. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.23.12 and v0.23.13 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.23.13** (processkit v0.25.8): Patch release: fixes 0.23.11-to-0.23.12 upgrades for generated configs that still place a moved tool under its old addon owner; apply now migrates misplaced addon tool entries to their unique current catalog owner before strict validation and comment refresh, while preserving hard errors for unknown tools.
  <https://github.com/projectious-work/aibox/releases/tag/v0.23.13>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox apply` was run for v0.23.13 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.23.13` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.23.13"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.23.13.
