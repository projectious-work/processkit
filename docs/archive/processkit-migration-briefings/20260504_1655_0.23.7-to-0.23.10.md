...
**Status:** pending
**aibox CLI:** v0.23.7 → v0.23.10
**processkit:** v0.25.7

## Summary

aibox has been updated from v0.23.7 to v0.23.10. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.23.7 and v0.23.10 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.23.8** (processkit v0.25.5): Patch release: fixes the native aibox Zellij status plugin artifact so Zellij can load its literal WASM entrypoints, restores readable theme-default foreground rendering for the key/status rows, and adds no-container E2E coverage for the export and foreground regressions.
  <https://github.com/projectious-work/aibox/releases/tag/v0.23.8>
- **v0.23.9** (processkit v0.25.6): Patch release: restores the shell-backed Zellij status rows as the default, hardens aibox-status against /proc races, fixes Yazi's edit action by using Zellij's editor action instead of injecting Vim commands, applies addon dependency fallback handling to `aibox up`, and integrates processkit v0.25.6 provider-neutral pk command projections.
  <https://github.com/projectious-work/aibox/releases/tag/v0.23.9>
- **v0.23.10** (processkit v0.25.7): Patch release: integrates processkit v0.25.7 model-routing content, migrates aibox.toml toward the current apiVersion/kind/metadata/image structure during apply, keeps generated config comments self-documenting, exposes image-slimming tool switches through addons, and finalizes generated-runtime release handling.
  <https://github.com/projectious-work/aibox/releases/tag/v0.23.10>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox apply` was run for v0.23.10 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.23.10` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.23.10"`
...
- [ ] Verify `/workspace/context/templates/processkit/v0.25.7/` snapshot directory exists
...
- [ ] Check `/workspace/context/migrations/pending/` for processkit content migration files:
      ...
      - **When reviewing template files like `AGENTS.md`:** do NOT diff the installed
        ...
        `diff context/templates/processkit/v0.25.7/AGENTS.md context/templates/processkit/v0.25.7/AGENTS.md`
...
processkit is tracking `version = "latest"` — installed: `v0.25.7` (upgraded from `v0.25.7` — aibox.toml now targets `vlatest`) (source: `https://github.com/projectious-work/processkit.git`).
...
- [ ] processkit version reference matches `aibox.lock` (`v0.25.7`)
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.23.10.
