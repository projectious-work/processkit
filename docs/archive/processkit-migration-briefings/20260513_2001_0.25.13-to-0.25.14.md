...
**Status:** pending
**aibox CLI:** v0.25.13 → v0.25.14
**processkit:** v0.26.5

## Summary

aibox has been updated from v0.25.13 to v0.25.14. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.25.13 and v0.25.14 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.25.14** (processkit v0.26.5): Patch release: integrates processkit v0.26.5; restores generated and image fallback Alt-word movement in Vim/readline; adds managed .inputrc runtime projection; carries tmux clipboard/terminal feature improvements into generated and image fallback configs; keeps Rust cache mounts from shadowing image-provided cargo/rustc shims; and keeps the stricter processkit schema/file-layout migration path clean under pk-doctor.
  <https://github.com/projectious-work/aibox/releases/tag/v0.25.14>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox apply` was run for v0.25.14 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.25.14` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.25.14"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.25.14.
