...
**Status:** pending
**aibox CLI:** v0.23.14 → v0.23.15
**processkit:** v0.25.8

## Summary

aibox has been updated from v0.23.14 to v0.23.15. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.23.14 and v0.23.15 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.23.15** (processkit v0.25.8): Patch release: fixes a 0.23.14 standardize-config regression where a blank [ai.harness.<name>] table could re-enable a harness after its controls were commented out, and restores the standard processkit skill list when canonical config rewrites encounter an empty skill include list.
  <https://github.com/projectious-work/aibox/releases/tag/v0.23.15>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox apply` was run for v0.23.15 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.23.15` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.23.15"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.23.15.
