...
spec:
  ...
  summary: |
    Static HTML comparison of six major AI providers (Anthropic, OpenAI, Google Gemini, Mistral, xAI Grok, DeepSeek) across subscription tiers, flagship model capability benchmarks (SWE-bench Verified, SWE-bench Pro, LiveCodeBench, community coding rating), context windows, API pricing (input/output, cache hit, batch discount), speed, and quota-burn ratios. Captured 2026-04-14 by the owner as input for the `team-creator` skill (FEAT-20260415_1505-TeamWeaver).
  criteria_index:
    cost:
    - subscription_price
    - input_per_million
