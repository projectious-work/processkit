...
**Status:** pending
**aibox CLI:** v0.26.5 → v0.26.7
**processkit:** v0.26.14

## Summary

aibox has been updated from v0.26.5 to v0.26.7. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.26.5 and v0.26.7 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.26.6** (processkit v0.26.14): Patch release: integrates processkit v0.26.14; updates processkit metadata to the new release and preserves tmux/Vim/cheatsheet and clipboard behavior consistency.
  <https://github.com/projectious-work/aibox/releases/tag/v0.26.6>
- **v0.26.7** (processkit v0.26.14): Patch release: integrates processkit v0.26.14; adds provider-neutral AI execution policy axes for filesystem, approval, and network behavior; adds per-harness execution overrides; maps execution policy to Codex settings generation; and keeps MCP permission configuration focused on tool allow/deny intent.
  <https://github.com/projectious-work/aibox/releases/tag/v0.26.7>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox apply` was run for v0.26.7 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.26.7` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.26.7"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.26.7.
