...
**Status:** completed
**aibox CLI:** v0.23.3 → v0.23.4
**processkit:** v0.25.3

## Summary

aibox has been updated from v0.23.3 to v0.23.4. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.23.3 and v0.23.4 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.23.4** (processkit v0.25.4): Patch release: integrates processkit v0.25.4 gateway stdio-proxy daemon startup fixes, wires Codex pre_tool_use hook generation for processkit compliance gating, adds [customization.zellij_status] presentation control, and repairs stale managed Zellij status layouts during runtime sync.
  <https://github.com/projectious-work/aibox/releases/tag/v0.23.4>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox apply` was run for v0.23.4 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.23.4` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.23.4"`
...
- [ ] Verify `/workspace/context/templates/processkit/v0.25.3/` snapshot directory exists
...
- [ ] Check `/workspace/context/migrations/pending/` for processkit content migration files:
      ...
      - **When reviewing template files like `AGENTS.md`:** do NOT diff the installed
        ...
        `diff context/templates/processkit/v0.25.3/AGENTS.md context/templates/processkit/v0.25.3/AGENTS.md`
...
processkit is tracking `version = "latest"` — installed: `v0.25.3` (upgraded from `v0.25.3` — aibox.toml now targets `vlatest`) (source: `https://github.com/projectious-work/processkit.git`).
...
- [ ] processkit version reference matches `aibox.lock` (`v0.25.3`)
...
Completed 2026-05-04. Current `aibox.lock` is at aibox `0.23.5`
with processkit `v0.25.4`, confirming this sequential CLI migration
has been superseded by the later successful `0.23.5` sync. No
processkit Migration entities remain pending or in progress.
`context/processes/` is absent in the current `v0.25.4` snapshot, so
that generated checklist item is stale for this project.
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.23.4.
