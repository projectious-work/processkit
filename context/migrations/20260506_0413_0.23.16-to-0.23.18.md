...
**Status:** pending
**aibox CLI:** v0.23.16 → v0.23.18
**processkit:** v0.25.8

## Summary

aibox has been updated from v0.23.16 to v0.23.18. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.23.16 and v0.23.18 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.23.17** (processkit v0.25.8): Patch release: installs Claude Code from Anthropic's signed apt repository with a stable /usr/local/bin/claude path, makes the native Zellij status/key-hint plugin the generated default, starts shell and lazygit tabs hot across layouts, bumps Zellij/Yazi/uv/Cargo dependencies, and improves release-state dependency and harness reporting with harness version-pin support.
  <https://github.com/projectious-work/aibox/releases/tag/v0.23.17>
- **v0.23.18** (processkit v0.25.8): Patch release: updates generated Yazi config and theme filetype rules for Yazi 26's url/mime matcher schema, provides writable XDG state mounts for lazygit and similar TUIs, and records follow-up backlog items for native Zellij plugin runtime diagnostics and host-phase runtime smoke tests.
  <https://github.com/projectious-work/aibox/releases/tag/v0.23.18>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox apply` was run for v0.23.18 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.23.18` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.23.18"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.23.18.
