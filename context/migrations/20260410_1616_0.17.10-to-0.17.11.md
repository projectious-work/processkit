...
**Status:** completed
**aibox CLI:** v0.17.10 → v0.17.11
**processkit:** vlatest

## Summary

aibox has been updated from v0.17.10 to v0.17.11. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Review the release notes for each version in this upgrade:
- https://github.com/projectious-work/aibox/releases/tag/v0.17.11

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox sync` was run for v0.17.11 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.17.11` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.17.11"`
...
processkit is pinned to `vlatest` (upgraded from `v0.8.0` — aibox.toml now targets `vlatest`) (source: `https://github.com/projectious-work/processkit.git`).
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.17.11.
