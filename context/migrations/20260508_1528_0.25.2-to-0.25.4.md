...
**Status:** pending
**aibox CLI:** v0.25.2 → v0.25.4
**processkit:** v0.25.8

## Summary

aibox has been updated from v0.25.2 to v0.25.4. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.25.2 and v0.25.4 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.25.3** (processkit v0.25.8): Patch release: fixes Yazi `e` editor handoff for tmux by targeting the existing Vim pane/window without spawning nested panes, returns focus to Yazi on `:q`, enables Vim mouse/no-wrap scrolling defaults, documents tmux status modes and element toggles, makes skill-finder discover deselected skills from the template catalog and lazy-install matches, adds SSH-first E2E companion guidance with uidmap preflight, and tightens host runtime smoke/persistence guardrails.
  <https://github.com/projectious-work/aibox/releases/tag/v0.25.3>
- **v0.25.4** (processkit v0.25.8): Patch release: repairs tmux release-host smoke probing after the managed tmux socket migration and fixes generated tmux status-right rendering by preserving the aibox runtime status segment.
  <https://github.com/projectious-work/aibox/releases/tag/v0.25.4>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox apply` was run for v0.25.4 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.25.4` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.25.4"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.25.4.
