...
**Status:** completed
**aibox CLI:** v0.23.2 → v0.23.3
**processkit:** v0.25.3

## Summary

aibox has been updated from v0.23.2 to v0.23.3. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.23.2 and v0.23.3 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.23.3** (processkit v0.25.3): Patch release: integrates processkit v0.25.3 model-spec/model-profile migrations and makes the Codex seccomp=unconfined bubblewrap fallback part of generated docker-compose.yml for downstream projects.
  <https://github.com/projectious-work/aibox/releases/tag/v0.23.3>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox apply` was run for v0.23.3 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.23.3` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.23.3"`
...
has been superseded by later successful syncs. The pending
processkit Migration entity emitted during this range was resolved via
migration-management MCP as `MIG-RUNTIME-20260504T060107`.
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.23.3.
