...
**Status:** pending
**aibox CLI:** v0.23.13 → v0.23.14
**processkit:** v0.25.8

## Summary

aibox has been updated from v0.23.13 to v0.23.14. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.23.13 and v0.23.14 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.23.14** (processkit v0.25.8): Patch release: makes per-harness [ai.harness.<name>] tables the canonical generated aibox.toml selector, adds opt-in `aibox apply --standardize-config` for schema-clean canonical rewrites, removes stale/deprecated generated comments, restores Yazi `e` cross-pane editor handoff, and refreshes docs around semantic AI harness configuration.
  <https://github.com/projectious-work/aibox/releases/tag/v0.23.14>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox apply` was run for v0.23.14 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.23.14` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.23.14"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.23.14.
