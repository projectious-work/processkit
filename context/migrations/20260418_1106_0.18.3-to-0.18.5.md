...
**aibox CLI:** v0.18.3 → v0.18.5
**processkit:** v0.17.0

## Summary

aibox has been updated from v0.18.3 to v0.18.5. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.18.3 and v0.18.5 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.18.4** (processkit v0.17.0): INCOMPLETE RELEASE — tag cut before the multi-version-upgrade fixes landed; Cargo.toml was also not bumped so the shipped binary self-reports as 0.18.3. Skip this version; use 0.18.5 or later.
  <https://github.com/projectious-work/aibox/releases/tag/v0.18.4>
- **v0.18.5** (processkit v0.18.1): catch-up release: completes the 0.18.4 work (multi-version upgrade gaps closed); bumps default processkit to v0.18.1 (hookEventName hotfix + src↔context parity); hook commands now use $CLAUDE_PROJECT_DIR so they work regardless of Claude Code's launch cwd; maintain.sh release now writes Cargo.toml + refreshes Cargo.lock before tagging.
  <https://github.com/projectious-work/aibox/releases/tag/v0.18.5>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox sync` was run for v0.18.5 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.18.5` and
...
- [ ] Verify `/workspace/context/templates/processkit/v0.17.0/` snapshot directory exists
...
- [ ] Check `/workspace/context/migrations/pending/` for processkit content migration files:
      ...
      - **When reviewing template files like `AGENTS.md`:** do NOT diff the installed
        ...
        `diff context/templates/processkit/v0.17.0/AGENTS.md context/templates/processkit/v0.17.0/AGENTS.md`
...
- [ ] processkit version reference matches `aibox.lock` (`v0.17.0`)
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.18.5.
