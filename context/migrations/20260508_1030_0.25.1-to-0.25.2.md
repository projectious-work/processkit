...
**Status:** pending
**aibox CLI:** v0.25.1 → v0.25.2
**processkit:** v0.25.8

## Summary

aibox has been updated from v0.25.1 to v0.25.2. Review each section below
and discuss action items with the project owner.

## Breaking Changes

Every release between v0.25.1 and v0.25.2 is listed below.                  Review each; treat entries tagged as breaking as action items.

- **v0.25.2** (processkit v0.25.8): Patch release: adds tmux-native `prefix ?` keybinding help popup, upgrades the two-line PowerKit status row with pane context and tmux mode detail, labels status metrics in aibox-status (CPU/LOAD/NET/MEM/DISK/LOG/OOM/PROC/AI/MCP/MIG/UP), fixes startup regression by targeting named tmux windows instead of `$session:1`, extends release/runtime caching (buildx registry cache refs, E2E Dockerfile dependency split, no podman-compose in runner), and documents provider endpoint base URL hints (ANTHROPIC_BASE_URL/OPENAI_BASE_URL/GEMINI_BASE_URL/MISTRAL_BASE_URL).
  <https://github.com/projectious-work/aibox/releases/tag/v0.25.2>

## Action Items

### Host actions (owner runs these outside the container)

- [ ] Owner: verify `aibox apply` was run for v0.25.2 — check `aibox.lock` at
      `/workspace/aibox.lock`: `[aibox].cli_version` should equal `0.25.2` and
...
- [ ] Read `/workspace/aibox.lock` — confirm `[aibox].cli_version = "0.25.2"`
...
Check https://github.com/projectious-work/aibox/issues for known issues with v0.25.2.
