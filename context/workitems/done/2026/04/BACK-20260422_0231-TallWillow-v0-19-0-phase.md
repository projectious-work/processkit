...
spec:
  ...
  description: |
    Replace `actor-profile` skill with `team-manager`. Clean break (no backward-compat per user directive).

    **Skill location**: `context/skills/processkit/team-manager/`

    **MCP tools**
    - Lifecycle: create_team_member, get_team_member, list_team_members, update_team_member, deactivate_team_member, reactivate_team_member
    - Name pool: reserve_name, release_name, list_available_names, suggest_name
    - Memory tree scaffolding: init_memory_tree(slug) — creates knowledge/, journal/, skills/, relations/, lessons/, private/ with .gitkeep and frontmatter templates
    - Export/import: export_team_member(slug) → tarball (A2A-signed card); import_team_member(tarball) with signature validation; respects export_policy (default-excludes journal/ and project-scoped)
    - Consistency: check_consistency(slug), check_all_consistency() — returns 10 check categories (see Phase 6 for pk-doctor wiring)

    **Name pool**: `data/name-pool.yaml` shipped with ~60 curated international English names (Alice, Aria, Ava, Cora, Emma, Eva, Hazel, Iris, Ivy, Kira, Lara, Lena, Lily, Luna, Maya, Mia, Nina, Nora, Ruby, Sara, Sophie, Zoe; Adam, Alex, Cole, Dean, Eli, Ethan, Felix, Finn, Henry, Ivan, Jack, Leo, Liam, Luke, Mark, Max, Milo, Noah, Oscar, Owen, Theo, Tim; Avery, Blake, Casey, Dana, Jamie, Jordan, Kai, Morgan, Quinn, Rae, Riley, Robin, Sage, Sam, Taylor). One-shot reservation.
