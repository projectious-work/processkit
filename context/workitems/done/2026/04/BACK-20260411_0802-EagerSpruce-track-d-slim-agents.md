...
spec:
  ...
  priority: medium
  description: |
    Reduce AGENTS.md to under 60 lines of core content — push domain-specific instructions into the skills that own them. Add explicit if/then skill guards:
    - If editing any file under context/ → check skill-finder first
    - If creating or transitioning a processkit entity → use the relevant MCP server, not hand edits

    Research shows root AGENTS.md content under 60 lines significantly improves adherence to the instructions that remain.
